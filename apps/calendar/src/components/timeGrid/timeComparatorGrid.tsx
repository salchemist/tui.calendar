...
interface Props {
  events: EventUIModel[];
  timeGridData: TimeComparatorGridData;
}

export function TimeComparatorGrid({ timeGridData, events }: Props) {
  const {
    isReadOnly,
  ...
  const totalUIModels = useMemo(
    () =>
      columns
        .map(({ date ,creatorName}) =>
          events
            .filter(isBetween(toStartOfDay(date), toEndOfDay(date)))
            .filter(event=>event.model.raw.creator.name===creatorName)
      ...
      <div className={classNames.scrollArea}>
        ...
        >
          ...
          {columns.map((column, index) => (
            <Column
              key={column.date.toString()+column.creatorName}
